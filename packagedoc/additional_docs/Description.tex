...
\begin{robotlog}
usage: TestResultDBAccess (RobotXMLResult to TestResultWebApp importer) [-h] [-v]
...
TestResultDBAccess imports XML result files (default: output.xml) generated by the
...
--UUID UUID          UUID used to identify the import and version ID on webapp.
                     If not provided TestResultDBAccess will generate an UUID for the
                     whole import.
...
\begin{robotlog}
TestResultDBAccess <output.xml> <localhost> <db\_user> <db\_pw> <db\_name>
\end{robotlog}

    Besides the executable file \pkg\, you can also run tool as a Python module
\begin{robotlog}
python -m TestResultDBAccess <output.xml> <localhost> <db\_user> <db\_pw> <db\_name>
\end{robotlog}
...
\begin{robotlog}
python -m TestResultDBAccess output.xml localhost testuser testpw testdb --UUID c7991c07-4de2-4d65-8568-00c5556c82aa --append
\end{robotlog}
